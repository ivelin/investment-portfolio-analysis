...
/** Shipped GET handler — OAuth broker callback. */
export async function oauthCallbackGet({
  request,
  params,
}: {
  request: Request;
  ...
  try {
    ...
    const { BROKERS } = await import("@/lib/portfolio/brokers/catalog");
    ...
    if (!bind.ok) {
      ...
      const reason =
        bind.reason === "no_session" ? "sign_in_required" : "session_mismatch";
      return Response.redirect(
    ...
    const mode = def.authKind === "remote_mcp" ? "remote_mcp" : "direct_oauth";
